...
1. Copy env template dan isi kredensial asli:
   ...
   ```
   Edit `.env` dan `prometheus/pve.yml` dengan host, user, token Proxmox, dan password admin Grafana. Edit juga `alertmanager/alertmanager.yml` dengan bot token dan chat_id Telegram asli (dua tempat: receiver `telegram-critical` dan `telegram-warning`). Ketiga file ini di-gitignore — jangan pernah di-commit.
